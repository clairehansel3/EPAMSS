...
Parameters::Parameters(const char* input_path, boost::mpi::communicator& world)
{
  ...
  EPAMSS_READ_PARAMETER(dict, rho_ion_si);
...
void Parameters::writeOutputFile(double seconds)
{
  try {
    ...
    EPAMSS_WRITE(file, rho_ion_si, "m^-3");
    ...
    EPAMSS_WRITE(file, rho_ion_div_n0, "");
...
void Parameters::computeDependentParameters(int processes)
{
  ...
  rho_ion_div_n0 = rho_ion_si / unperturbed_plasma_density_si;
  ...
  omega_on_axis_initial = omega_off_axis * (rho_ion_div_n0 + delta);
  if (omega_on_axis_initial < 25) {
    ...
    max_scattering_r_div_a_initial = std::sqrt(std::sqrt(rho_ion_div_n0 * omega_off_axis / (25 - delta * omega_off_axis)) - 1);
  }
  sigma_r = 0.5 * bennett_radius_initial * std::sqrt(rho_ion_div_n0);
  sigma_r_prime_initial = std::sqrt(boost::math::constants::pi<double>() * classical_electron_radius * bennett_radius_initial_si * bennett_radius_initial_si * ion_atomic_number * rho_ion_si / (2 * gamma_initial));
