...
// of particles in the beam, the means of x, x', y, y', and the 4x4 sigma matrix
// corresponding to the variables x, x', y, and y'.
...
public:
  ...
  // doubles and written in order to the file: mean(x), mean(x'), mean(y),
  // mean(y'), cov(x, x), cov(x, x'), cov(x, y), cov(x, y'), cov(x', x'),
  // cov(x', y), cov(x', y'), cov(y, y), cov(y, y'), cov(y', y')

private:

  std::array<double, 4> m_means;
  // The means of x, x', y, y'.

  std::array<double, 10> m_covariance_matrix;
  // The entries of the 4x4 covariance matrix for the variables x, x', y, y'
